...
* **deadLetter**, string
  ...
  When a job fails after all retries, if the queue has a `deadLetter` property, the job's payload will be copied into that queue. The copy is a job on the dead letter queue and runs under *that* queue's configuration. Retry, retention, expiration, and heartbeat all come from the dead letter queue, not the original job. What travels with the job is its identity: `priority`, `singletonKey`, and `group`, so ordering weight and group concurrency limits still apply. The dead-lettered job also records where it came from via the `sourceName`, `sourceId`, `sourceCreatedOn`, `sourceRetryCount`, and `sourceOutput` fields (`sourceOutput` is the original job's failure output; the copy's own `output` starts empty), plus `sourceRootId`, the first job in the chain, which survives any number of redrives. [`redrive()`](jobs#redrivename-options) uses `sourceName` to route each job back to the queue it failed on.
...
* `deltaSeconds`: how many seconds the three counts cover. Monitor passes are not evenly spaced (a deferred or missed pass covers several intervals), so compute a rate as `completedDelta / deltaSeconds * 60`, not by dividing by the bucket width. `null` on a queue's first counted pass, and wherever the counts are `null`. A queue that went more than two hours without being counted starts a fresh window rather than reporting the whole gap on one snapshot.
* `deltaOn`: when the interval the three counts cover ends; it starts `deltaSeconds` earlier. This is a minute behind `capturedOn`, so plot the counters at `deltaOn` and the gauges at `capturedOn`. In a bucketed series the counters are already placed by it: gauges are bucketed by `capturedOn` and counters by `deltaOn`, so the two line up, and the newest bucket's counters are `null` until the pass that counts its minute has run. A counter whose bucket holds no capture is folded into the bucket of the newest capture before it, so every bucket returned has real gauges. `null` wherever the counts are.

The counters are eventually consistent rather than up to the second. A job is counted by the time pg-boss stamped on it, which is the start of the transaction that created or finished it, and that row only becomes visible when the transaction commits. So each window ends a minute behind the pass: a transaction that commits within a minute of starting is always counted, whatever database role it runs as and on every backend, and its job shows up in the first pass after its stamp is a minute old. Work done inside a longer transaction, such as a [transactional worker](./workers.md#workname-options-handler) whose handler runs for minutes, is covered on PostgreSQL by holding the window back further, to the start of the oldest transaction still open that the monitoring role can see in `pg_stat_activity`. Any open transaction does this, including a read-only report or a backup, and while one stays open, counting for every queue waits on it, up to an hour. A transaction open longer than that holds the window an hour behind for as long as it stays open, and jobs it commits more than an hour after starting are not counted. One under another database role whose start time this role cannot read does not hold the window back beyond the minute, and jobs it commits later than that are not counted. CockroachDB and YugabyteDB don't expose open transactions this way, so there the minute is the whole allowance.
