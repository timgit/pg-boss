...
A job is counted by the time pg-boss stamped on it, which is the start of the transaction that created or finished it. So work done inside a longer transaction, such as a [transactional worker](./workers.md#workname-options-handler) or a `send()` within your own transaction, is counted in the first pass after it commits, not in the pass it started in. On PostgreSQL each window ends at the start of the oldest transaction still open, so the late commit is counted when it lands rather than skipped; while such a transaction stays open, counting for every queue waits on it. It is never held back by more than an hour: a transaction open longer than that delays counting by an hour for as long as it stays open, and jobs it commits after that are not counted. A transaction belonging to another database role, whose start time this role cannot read, does not hold the window back, and jobs it commits late are not counted either. CockroachDB and YugabyteDB don't expose open transactions this way, so there a job committed after the pass that followed its transaction's start is not counted.
