...
* **persistQueueStats**, bool, default false
  ...
  It also turns on throughput counting: each snapshot records how many jobs arrived, completed and failed in the interval ending a minute before the pass (`createdDelta`, `completedDelta`, `failedDelta`), with how many seconds that covers (`deltaSeconds`) and when it ended (`deltaOn`), which no depth count can answer. The minute's lag is what lets a job written inside a transaction be counted once it commits; see [`getQueueStats()`](./queues.md#getqueuestatsname-options). This makes the monitor's aggregate more expensive: on Postgres 18 it added 3-13% on a 2-million-row queue (~755ms to ~780-855ms) and ~16% across 1,000 small queues (~208ms to ~241ms per pass).
