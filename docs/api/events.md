...
| `transactional_pool_headroom` | A [`transactional`](./workers.md#workname-options-handler) worker was registered on a pool with no connections to spare. Each handler in flight holds one for its own duration, so at this size the fetches, the failures, and maintenance have nothing left to run on and wait out `connectionTimeoutMillis` instead. Emitted from `work()`, once per call. Event only, never written to the `warning` table | `type`, `max`, `transactionalWorkers` |
| `transaction_timeout_probe` | pg-boss asked the server which transaction timeout it recognises and the statement failed, so transactional handlers run with no database-side bound (see [`transactionTimeoutSeconds`](./workers.md#workname-options-handler)). Every other bound still applies: the handler's own timeout, the rollback deadline, and `expireInSeconds`. The probe is retried on the next transactional batch; the warning is emitted once per process. Event only, never written to the `warning` table | `type` |
...
The table records episodes: a condition the database entered and can leave again. Two types are left out of it and emitted as events and nothing else. `transactional_pool_headroom` reports a fixed relationship between `max` and the workers you registered, which no restart or later pass resolves. `transaction_timeout_probe` fires because a statement failed, so a write to the `warning` table is the least likely thing to succeed at that moment.
