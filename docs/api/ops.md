...
If the required database objects do not exist in the specified database, **`start()` will automatically create them** at the current schema version. There is no in-place upgrade from an older installed schema version: if pg-boss finds a schema older than the version this release ships, `start()` throws rather than migrating it in place.
