...
function createQueueFunction (schema: string) {
  return `
    ...
    BEGIN
      ...
      ELSIF options->>'policy' = '${QUEUE_POLICIES.singleton_strict_fifo}' THEN
        EXECUTE ${schema}.job_table_format($cmd$${createIndexJobPolicySingletonStrictFifo(schema)}$cmd$, tablename);
      END IF;
