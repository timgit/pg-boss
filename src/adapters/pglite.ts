...
// Minimal structural type for an `@electric-sql/pglite` instance, so bun-boss does not take a
...
export function fromPglite (pglite: PGliteLike): IDatabase {
  ...
  const run = async (text: string, values?: unknown[]) => {
    ...
    // middle isn't lost behind a trailing COMMIT. This mirrors how bun-boss unwraps the array that
  ...
  // the same instance both NOTIFYs (via bun-boss's inlined pg_notify) and delivers to listeners.
