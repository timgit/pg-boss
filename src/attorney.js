...
function validateRetryConfig (config) {
  ...
  assert(!('maxRetryDelay' in config) || config.maxRetryDelay === null || config.retryBackoff === true, 'maxRetryDelay can only be set if retryBackoff is true')
  assert(!('maxRetryDelay' in config) || config.maxRetryDelay === null || (Number.isInteger(config.maxRetryDelay) && config.maxRetryDelay >= 0), 'maxRetryDelay must be an integer >= 0')
}
