...
| **noAdvisoryLocks** | `pg_advisory_xact_lock` serializing schema creation, create/delete-queue, stats cache | Drop the lock; rely on the wrapping transaction + idempotent DDL (`IF NOT EXISTS` / `ON CONFLICT`) (`plans.locked`) | Concurrent schema/maintenance ops from multiple instances can race — risk of transient contention errors instead of clean serialization. |
...
**Who trips which:** the `sqlite` profile sets all seven; the `postgres` and `pglite`
profiles set none. On plain Postgres the flags are exercised through the `__test__`
hooks (`NO_SKIP_LOCKED_NO_CTE=true` forces the two runtime flags suite-wide).
